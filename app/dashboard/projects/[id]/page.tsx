...
export default function ProjectPage() {
  ...
  const processSearchQueue = useCallback(async () => {
    if (isProcessingQueue || searchQueue.length === 0 || currentKeywordIndex >= searchQueue.length) {
      if (currentKeywordIndex >= searchQueue.length) {
    ...
    const keyword = searchQueue[currentKeywordIndex];
    const isLastKeyword = currentKeywordIndex === searchQueue.length - 1;

    try {
      const response = await fetch(`/api/products/${id}/find-leads`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ action: 'getNewestLeads', keyword, isLastKeyword }),
      });
      ...
      // Update remaining lead finds based on the response
      if (isLastKeyword && data.remainingLeadFinds !== undefined) {
        setRemainingLeadFinds(data.remainingLeadFinds);
      }
    } catch (error) {
  ...
  }, [id, searchQueue, setProgress, toast, currentKeywordIndex]);
  ...
  }, [processSearchQueue, isProcessingQueue, currentKeywordIndex]);
  ...
  const getNewestLeads = useCallback(async () => {
    ...
    setCurrentKeywordIndex(0);
  }, [product, remainingLeadFinds, router, toast]);
